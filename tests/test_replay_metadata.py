...
@pytest.mark.skipif(
    not (
        Path("strategy_a_breeze_baseline_report.json").exists()
        and Path("data/market/historical.db").exists()
        and Path("data/instruments/instruments.db").exists()
    ),
    reason="Breeze baseline artifacts are not present",
)
def test_breeze_baseline_signal_ids_are_reproducible():
    async def run():
        report = json.loads(Path("strategy_a_breeze_baseline_report.json").read_text(encoding="utf-8"))
        expected = {row["id"] for row in report["rows"]}
        conn = sqlite3.connect("data/market/historical.db")
        dates = [
            row[0]
